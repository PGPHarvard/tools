...
def main():
   ...
   newnameBiome = "PGP_Microbiome_collection"
   ownerUUID = "su92l-j7d0g-nsjiqjm7jne7zgr"
   destBiome = findCollection(newnameBiome,ownerUUID)
     ...
     if ccnum in source_dict:
        ...
        if ctype == 'Microbiome':
          for items in cc:
             ...
             if not destBiome.exists(target):
               print("does not exist, copy "+ target)
               destBiome.copy("./"+items,target_path=target,source_collection=cc,overwrite=True)
          destBiome.save()
        else:
